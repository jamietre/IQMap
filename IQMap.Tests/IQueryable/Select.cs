...
namespace IQMap.Tests.IQueryable
{
    [TestClass]
    public class Select
    {
        ...
        public void Simple()
        {
            ...
            //var dogs = from dog
            //           in IQ.From<Dog>()
            //           where dog.Breed == "Terrier"
            //           select dog;
            //Assert.AreEqual(2, dogs.Count());
            //Assert.AreEqual("Terrier",dogs.First().Breed);
            
            var dog = IQ.From<Dog>(2).First();

            Assert.AreEqual("SELECT TOP 1 PK,Breed,Age,Id,Name,Weight FROM animals WHERE PK=@PK", TC.Controller.LastQuery.GetQuery(), "Query looks good");
            ...
            Assert.AreEqual(2, TC.Controller.LastQuery.Parameters.ElementAt(0).Value, "Value for parm is correct");
