...
namespace IQMap.Tests
{
    public class TestDataReader: IDataReader
    {
        ...
        public void Generate(int rows)
        {
            for (int i = 0; i < rows; i++)
            {
                ...
                for (int j = 0; j < FieldsList.Count; j++)
                {
                    if (i == 0)
                    {
                        rowData.Add(GetPredefinedData(TypesList[j]));
                    }
                    else
                    {
                        rowData.Add(GetRandomData(TypesList[j]));
                    }
                }
                Data.Add(rowData);
            }
        }
        protected List<string> FieldsList;
        protected List<Type> TypesList;
        ...
        private int GetFieldIndex(string fieldName)
        {
            ...
            {
                if (FieldsList[i].ToLower() == fieldName)
                {
                    return i;
                }
            }
        ...
        {
            return FieldsList.IndexOf(name.ToLower());
        }
